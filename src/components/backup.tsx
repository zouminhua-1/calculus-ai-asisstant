...
const ChatWindow: React.FC = () => {
  ...
  const fetchHistoryList = async () => {
    try {
      ...
      const res = await fetch(
        `/cros/conversations?user=deep-chat-user&limit=20`,
        {
  ...
  const loadConversation = async (convId: string) => {
    ...
    try {
      const res = await fetch(
        `/cros/messages?user=deep-chat-user&conversation_id=${convId}`,
        {
          headers: { Authorization: `Bearer ${DIFY_API_KEY}` },
        }
      );
  ...
  const deleteConversation = async (e: React.MouseEvent, convId: string) => {
    ...
    try {
      const res = await fetch(`/cros/conversations/${convId}`, {
        ...
        body: JSON.stringify({ user: 'deep-chat-user' }),
      });
  ...
  const requestHandler = async (body: any, signals) => {
    ...
    try {
      ...
      if (userFiles && userFiles.length > 0) {
        ...
        formData.append('user', 'deep-chat-user');
    ...
    <div className="flex h-[100dvh] w-full bg-white overflow-hidden text-slate-900 transition-colors">
      ...
      <AnimatePresence>
        {isDrawerOpen && (
          <>
            <motion.div
              /* ...遮罩... */ onClick={() => setIsDrawerOpen(false)}
            />
            <motion.div
              /* ...容器... */ className="fixed inset-y-0 left-0 w-[85%] max-w-sm bg-[#171717] text-white z-[70] shadow-2xl flex flex-col"
            >
              {/* Header 部分 */}
      ...
      <div className="flex-1 flex flex-col w-full relative">
        ...
        <main className="flex-1 overflow-hidden relative">
          <DeepChat
            ...
            avatars={{
              user: {
                src: doctor, // 替换为你 static 目录下的用户头像路径
                styles: { container: { width: '35px', height: '35px' } },
              },
              ai: {
                src: robot, // 替换为你 AI 助手的头像路径
                styles: { container: { width: '35px', height: '35px' } },
              },
            }}
